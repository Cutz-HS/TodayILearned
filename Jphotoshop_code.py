...
def loadImage(fname):
    ...
    inH = inW = int(math.sqrt(fsize)) # size
    inImage = np.zeros([inH, inW], dtype=np.int32) # 영행렬 미리 만들어 index로 쉽게 처리
    fp = open(fname, 'rb')
    for  i  in range(inH) :
        for  k  in  range(inW) :
            inImage[i][k] =  int(ord(fp.read(1))) # inImage에 data 넣기
    fp.close()
...
def display():
    ...
    if gif == True: # gif 신호등이 켜지면 gif 함수로
        display_gif()
        return
    if  canvas != None: # 기존 canvas가 존재하면, canvas 초기화
        canvas.destroy()
    if pLabel != None: # 복사본이 이미 존재하면, 초기화
        pLabel.destroy()
    window.geometry(str(outH) + 'x' + str(outW)) # window 창 생성
    canvas = Canvas(window, width=outW, height=outH)
    paper = PhotoImage(width=outW, height=outH)
    canvas.create_image((outW/2, outH/2), image=paper, state='normal')
    def putPixel() : # threading으로 실시간 업로딩
...
def merge_gif():
    ''' 
    gif 파일을 하나 더 입력받아서 두 이미지를 합성 (RGB 산술 평균) 
    '''
    global window, canvas, paper, filename, inImage, outImage, inW, inH, outW, outH, photo
    outW, outH = inW, inH
