...
### Q2 : mySQL 대용량 CSV 추가 ###
### Q3 : 테이블 별 별도의 CSV 생성 ###
### Q4 : mySQL 테이블 별 별도의 CSV 생성 ###

'''
mysql -u root -p
CREATE USER 'HS'@'%' IDENTIFIED BY '1111';
CREATE DATABASE cutzDB;
CREATE TABLE employee ( eNum INT, eName CHAR(5), eClass CHAR(5), eYear INT(5), salary BIGINT);
GRANT ALL PRIVILEGES ON cutzDB.* TO 'HS'@'%';
INSERT INTO employee VALUES(001, 'SDK', 'associate', 2015, 100000);
INSERT INTO employee VALUES(002, 'SDKj', 'Manager', 2013, 200000);
INSERT INTO employee VALUES(003, 'SDG', 'Director', 2011, 300000);
INSERT INTO employee VALUES(004, 'SDz', 'GM', 2008, 400000);
INSERT INTO employee VALUES(005, 'Cutz', 'CEO', 2000, 777777777);
'''
...
def sqliteData01() :
    ...
    con = sqlite3.connect('d:/data/userDB')  # 데이터베이스 지정(또는 연결)
...
def mySqlData01() :
    global csvList, input_file
    csvList = []
    con = pymysql.connect(host='192.168.226.131', user='root', password='1234', db='userDB', charset='utf8')  # 데이터베이스 지정(또는 연결)
    cur = con.cursor()
    sql = "SHOW tables;"
    cur.execute(sql)
    tableNameList = []
...
def mySqlData02():
    global csvList, input_file
    con = pymysql.connect(host='192.168.226.131', user='root', password='1234', db='userDB', charset='utf8')
    cur = con.cursor()
    headerList = []
...
mainMenu.add_cascade(label='대용량 데이터 처리 자동화', menu=bigDataMenu)
bigDataMenu.add_command(label='대용량 CSV --> SQLite', command=bigData01)
bigDataMenu.add_command(label='대용량 CSV --> mySQL', command=bigData02)
bigDataMenu.add_command(label='대용량 SQLite --> CSV', command=bigData03)
bigDataMenu.add_command(label='대용량 mySQL --> CSV', command=bigData04)
