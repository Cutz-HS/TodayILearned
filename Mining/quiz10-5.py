...
def raw2csv(num):
    ...
    output_file = "d:/data/Etc_Raw/" + basename[:basename.index('.')] + ".csv"
    with open(output_file, 'w', newline='') as filewriter:
        csvWriter = csv.writer(filewriter)
        csvWriter.writerow(header)
        for row in range(inW):
            for col in range(inH):
                data = inImage[row][col]
...
rawMenu = Menu(mainMenu)
mainMenu.add_cascade(label='RAW <--> CSV', menu=rawMenu)
rawMenu.add_command(label='CSV 내보내기', command=lambda: raw2csv(1))
rawMenu.add_command(label='CSV 셔플 내보내기', command=lambda: raw2csv(2))
window.mainloop()
