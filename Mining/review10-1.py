...
def mySqlData01() :
    global csvList, input_file
    csvList = []
    con = pymysql.connect(host='192.168.226.128', user='cutz', password='1111', db='cutzDB', charset='utf8')  # 데이터베이스 지정(또는 연결)
    cur = con.cursor()
    sql = "SELECT table_name FROM information_schema.tables WHERE table_schema = 'cutzDB'"
    cur.execute(sql)
    tableNameList = []
    while True:
        row = cur.fetchone()
...
def mySqlData02():
    global csvList, input_file
    con = pymysql.connect(host='192.168.226.128', user='HS', password='1111', db='cutzDB', charset='utf8')
    cur = con.cursor()
    headerList = []
